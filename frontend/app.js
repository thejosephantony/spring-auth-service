...
async function verificarEExibirPainel() {
    try {
        ...
        if (user.role === 'ADMIN') {
            ...
            document.getElementById('admin-email-display').innerText = `${user.email} (ADMIN)`;
            carregarUsuarios();
        } else {
    ...
    try {
        ...
        usuarios.forEach(user => {
            ...
            tr.innerHTML = `
                ...
                <td class="text-right">
                    <button class="btn-action-edit" onclick="editarUsuario(${user.id})">Editar</button>
...
async function editarUsuario(id) {
    ...
    try {
        ...
        if (!response.ok) throw new Error('Erro ao atualizar usuário. Verifique os dados.');
...
// AGORA SIM: FORA DE QUALQUER OUTRA FUNÇÃO (ESCOPO GLOBAL)
async function editarMeuPerfil() {
    const novoNome = prompt("Digite o seu novo nome:");
    if (novoNome === null) return;
    
    const novoEmail = prompt("Digite o seu novo e-mail:");
    if (novoEmail === null) return;
    
    const novaSenha = prompt("Digite sua nova senha (mínimo 6 caracteres, ou deixe em branco para não alterar):");
    ...
    try {
        ...
        if (!response.ok) throw new Error('Erro ao atualizar perfil. Verifique se o e-mail já existe.');

        alert('Perfil atualizado com sucesso! Faça login novamente com sua nova senha se houver alteração.');
        fazerLogout();
    } catch (error) {
        alert(error.message);
    }
